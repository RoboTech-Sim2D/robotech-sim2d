...
{
    std::cerr << "Coach: Trying to load team graphic" << std::endl;
    
    if ( config().teamGraphicFile().empty() )
    {
        std::cerr << "Coach: Using embedded logo from team_logo.xpm" << std::endl;
        M_team_graphic.createXpmTiles( robotech_logo );
    }
    else
    {
        std::cerr << "Coach: Loading logo from file: " << config().teamGraphicFile() << std::endl;
        M_team_graphic.readXpmFile( config().teamGraphicFile().c_str() );
    }
    
    std::cerr << "Coach: Team graphic loaded. Tile count: " << M_team_graphic.tiles().size() << std::endl;
}
else
{
    std::cerr << "Coach: Team graphic is disabled in configuration" << std::endl;
}
...
{
    std::cerr << "Coach: Attempting to send team graphic. Tiles: " 
              << M_team_graphic.tiles().size() 
              << ", OK set: " << teamGraphicOKSet().size() << std::endl;
          ...
          ++tile )
    {
        ...
        {
            std::cerr << "Coach: Sending tile at position (" 
                      << tile->first.first << "," << tile->first.second << ")" << std::endl;
                      
            if ( ! doTeamGraphic( tile->first.first,
                                  tile->first.second,
                                  M_team_graphic ) )
            {
                std::cerr << "Coach: Failed to send tile" << std::endl;
                break;
            }
            ++count;
        }
    ...
    else if (M_team_graphic.tiles().size() > 0 && count == 0)
    {
        std::cerr << "Coach: No tiles were sent, all tiles already in OK set" << std::endl;
    }
}
