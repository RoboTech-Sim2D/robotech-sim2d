...
#include <fstream>   // guard de carga del golie DNN
#include <iostream>
...
bool Bhv_GoalieBasicMove::doHeliosGolie(PlayerAgent *agent)
{
    ...
    static DeepNueralNetwork * golie_dnn = nullptr;
    static bool load_dnn = false;
    if(!load_dnn){
        load_dnn = true;
        // Guard RoboTech: ReadFromKeras sobre archivo ausente CRASHEA (mismo
        // bug que el unmark DNN). Sin pesos → doHeliosGolie desactivado y el
        // portero cae al resto del flujo (MarliK/posicionamiento normal).
        std::ifstream test("./helios_golie_origin");
        if(!test.good()){
            std::cerr << "[GolieDNN] ./helios_golie_origin no encontrado — doHeliosGolie desactivado" << std::endl;
        } else {
            test.close();
    ...
    if(golie_dnn == nullptr){
        return false;   // sin pesos → siguiente rama del portero
    }
    MatrixXd input(2,1);
