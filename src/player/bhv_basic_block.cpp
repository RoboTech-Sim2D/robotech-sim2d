...
/*
    ...
    Phase 5: refactored get_blockers/get_best_blocker to static WorldModel-based
    methods; added who_is_blocker() for use by bhv_mark_decision_greedy.
*/
...
bool Bhv_BasicBlock::execute( PlayerAgent * agent )
{
    ...
    {
        dlog.addText( Logger::TEAM, __FILE__": block→intercept" );
        Body_Intercept().execute( agent );
        agent->setNeckAction( new Neck_OffensiveInterceptNeck() );
        return true;
    }

    auto tm_blockers = get_blockers( wm );

    int self_unum = wm.self().unum();
    if ( tm_blockers.empty()
      || std::find( tm_blockers.begin(), tm_blockers.end(), self_unum ) == tm_blockers.end() )
    {
        last_block_cycle = -1;
        return false;
    }

    auto best = get_best_blocker( wm, tm_blockers );
    if ( best.first != self_unum )
    {
        last_block_cycle = -1;
        return false;
    }
    ...
    double safe_dist = ( wm.self().pos().dist( target_point ) > 15 ) ? 5.0 : 2.0;
    ...
    dlog.addText( Logger::TEAM, __FILE__": Bhv_BasicBlock target=(%.1f %.1f)",
                  target_point.x, target_point.y );
    agent->debugClient().addMessage( "BasicBlock" );
    agent->debugClient().setTarget( target_point );

    if ( ! Body_GoToPoint( target_point, 0.5, 100, -1, 100, false, 25, 1.0, false )
               .execute( agent ) )
    {
...
// ── who_is_blocker ───────────────────────────────────────────────────────────
int Bhv_BasicBlock::who_is_blocker( const WorldModel & wm )
{
    auto tm_blockers = get_blockers( wm );
    if ( tm_blockers.empty() ) return 0;
    auto best = get_best_blocker( wm, tm_blockers );
    return best.first;  // 0 if no one can block in time
}
...
std::vector<int> Bhv_BasicBlock::get_blockers( const WorldModel & wm )
{
    ...
    {
        if ( tm->isGhost() ) continue;
        if ( tm->unum() < 1 || tm->unum() > 11 ) continue;
        if ( tm->goalie() ) continue;
        if ( tm->isTackling() ) continue;
        if ( tm->pos().dist( ball_inertia ) > 40 ) continue;

        Vector2D tm_home_pos = Strategy::i().getPosition( tm->unum() );
        if ( tm_home_pos.dist( ball_inertia ) > 40 ) continue;

        if ( tm->unum() <= 5 )
        {
            double tm_defense_line_x = 0.0;
            for ( int i = 2; i <= 5; ++i )
            {
                const AbstractPlayerObject * dtm = wm.ourPlayer( i );
                if ( dtm && dtm->unum() > 0 && dtm->pos().x < tm_defense_line_x )
                    tm_defense_line_x = dtm->pos().x;
            }
            if ( ball_inertia.x > -30.0
              && ball_inertia.x > tm_home_pos.x + 10.0
              && ball_inertia.x > tm_defense_line_x + 10.0 )
                continue;
        }
...
#ifdef DEBUG_BLOCK
    for ( auto & b : tm_blockers )
        dlog.addText( Logger::BLOCK, "- tm %d is added as blocker", b );
...
#ifdef DEBUG_BLOCK
        dlog.addText( Logger::BLOCK, "## cycle=%d ball=(%.1f,%.1f)",
                      cycle, ball_inertia.x, ball_inertia.y );
        dlog.addCircle( Logger::BLOCK, ball_inertia, 0.5, 255, 0, 0, false );
#endif

        for ( auto & tm_unum : tm_blockers )
        {
            const AbstractPlayerObject * tm = wm.ourPlayer( tm_unum );
            if ( ! tm ) continue;
            const int pos_count = tm->posCount();
            if ( pos_count > 8 ) continue;

            Vector2D tm_pos    = tm->pos() + tm->vel();
            double uncertainty = pos_count * 0.3;
            double dist        = ball_inertia.dist( tm_pos ) + uncertainty;
            int dash_step = tm->playerTypePtr()->cyclesToReachDistance( dist ) + 2;

            if ( dash_step <= cycle )
            {
#ifdef DEBUG_BLOCK
                dlog.addText( Logger::BLOCK, "$$$$ tm=%d can block at cycle=%d",
                              tm_unum, cycle );
...
AngleDeg Bhv_BasicBlock::dribble_direction_detector( Vector2D dribble_pos )
{
    ...
    for ( double dir = -180.0; dir < 180.0; dir += 10.0 )
    {
        ...
        if ( target.absX() > ServerParam::i().pitchHalfLength() ) continue;
        if ( target.absY() > ServerParam::i().pitchHalfWidth()  ) continue;

        double score = -target.x
                     + std::max( 0.0, 40.0 - target.dist( Vector2D( -50.0, 0.0 ) ) * 2.0 );
        if ( score > best_score )
        {
            best_score = score;
