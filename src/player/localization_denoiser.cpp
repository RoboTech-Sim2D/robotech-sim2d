...
//    update world model!!
    ...
    for (auto &p: wm_not_const.mutableTeammatesFromSelf()) {
        if (p == nullptr)
            continue;
        if (p->unum() <= 0)
            continue;
        if(teammates[p->unum()]->player_seen(p) || teammates[p->unum()]->player_heard(wm, p))
            if (teammates[p->unum()]->average_pos.isValid()){
                const_cast<rcsc::PlayerObject*>( &*p )->denoiserOverridePos( teammates[p->unum()]->average_pos ); // (port)
            }
    }
    for (auto &p: wm_not_const.mutableOpponentsFromSelf()) {
        if (p == nullptr)
            continue;
        if (p->unum() <= 0)
            continue;
        if(opponents[p->unum()]->player_seen(p) || opponents[p->unum()]->player_heard(wm, p))
            if (opponents[p->unum()]->average_pos.isValid()){
                const_cast<rcsc::PlayerObject*>( &*p )->denoiserOverridePos( opponents[p->unum()]->average_pos ); // (port)
            }
    }
}
...
void LocalizationDenoiser::debug(PlayerAgent * agent) {
    ...
    for (auto & p: agent->world().allPlayers()){
        // (port) M_base_pos no existe en nuestra librcsc — círculo de debug omitido
