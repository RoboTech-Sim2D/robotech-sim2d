...
#include "planner/shoot_generator.h"


#include "strategy.h"
#include "bhv_basic_block.h"
#include "bhv_tackle_intercept.h"
#include "planner/bhv_pass_kick_find_receiver.h"
...
Body_InterceptPlan::execute(PlayerAgent *agent) {
    ...
    {
        int self_tackle_min = wm.interceptTable().selfStep();
    ...
    if (self_min < opp_min + 1) {
        ...
        if(wm.ball().pos().dist(Vector2D(52,0)) < 20){
            Vector2D shoot_tar = Vector2D(52.5, 0.0); // (port) get_best_shoot es del fork
            ...
            if (!chain_graph.getAllChain().empty()){
                ...
                switch (first_action.category()) {
                    ...
                    case CooperativeAction::Pass: {
                        ...
                        if (self_min == 1) {
                            // (port) doSayPrePass no existe en nuestro planner - anuncio omitido
                        }
                        break;
                    }
                }
    ...
    if (wm.ball().pos().x > -15 || wm.lastKickerSide() == wm.ourSide()) {
        if (!tm_drible && !tm_pass) {
            if (self_min <= 1) {
                if (!Body_Intercept(false, face).execute(agent)){
                ...
                if (self_min <= mate_min) {
                    if (self_min < opp_min + dif) {
                        if (self_min <= opp_min) {
                            ...
                            if (!M_called_from_block && Bhv_BasicBlock().execute(agent)) {
