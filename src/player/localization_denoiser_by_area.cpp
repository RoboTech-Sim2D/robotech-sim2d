...
void PlayerPositionConvex::init() {
    ...
    for (int i = 0; i < PlayerParam::i().playerTypes(); i++){
        ...
        for(int dash_step = 1; dash_step <= N_DASH; dash_step++){
            const double max_dash_dist = ptype->dashDistanceTable()[dash_step - 1] /* (port) tabla omni del fork -> recta = cota sup segura */;
        ...
        for (int angle_step = 0; angle_step < N_ANGLE; angle_step++) {
            ...
            for(int dash_step = 1; dash_step <= N_DASH; dash_step++){
                ...
                if (omni_dash_speed + accel > ptype->realSpeedMax())
                    accel = ptype->realSpeedMax() - omni_dash_speed;
                ...
                if (dash_step > 1 && can_turn_in_one_cycle){
                    ...
                    if (turn_dash_speed + accel > ptype->realSpeedMax())
                        accel = ptype->realSpeedMax() - turn_dash_speed;
                    ...
                    accel = turn_dash_max_accel;
                    if (turn2_dash_speed + accel > ptype->realSpeedMax())
                        accel = ptype->realSpeedMax() - turn2_dash_speed;
...
void PlayerPredictedObjArea::update_candidates(const WorldModel &wm, const PlayerObject *p) {
    ...
    double seen_dist = rpos.r(); // (port) seen_dist() es del fork
...
void PlayerPredictedObjArea::update_by_hear(const WorldModel &wm, const PlayerObject *p){
    ...
    for (auto & a: memory){
        ...
        if (unum == p->unum()){
            heard_pos = a.pos_;
            error_dist = 0.25; // (port) pos_count_ es del fork; oido reciente
            heard_body = a.body_;
            heard_pos_count = 0; // (port) pos_count_ es del fork
