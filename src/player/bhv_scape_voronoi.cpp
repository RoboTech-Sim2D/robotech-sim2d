...
bool go_to_target(PlayerAgent * agent, Vector2D target){
    ...
    agent->setNeckAction( new Neck_TurnToBallOrScan( 0 ) ); // (port) NeckDecision es del fork
...
bool IntentionScape::finished( const rcsc::PlayerAgent * agent ){
    ...
    if(wm.self().pos().x > wm.offsideLineX() - 0.3)
        return true;
    if(Strategy::i().isPersonalDefenseSituation(wm, wm.self().unum()))
        return true;
...
bool bhv_scape_voronoi::execute(PlayerAgent *agent) {
    ...
    // PERF RoboTech (2026-07-11): la selección de carrera (Voronoi + evaluar
    // candidatos) generó 149 picos >70ms/tanda con varios atacantes eligiendo
    // en el MISMO ciclo ("jugadores pensando", visto por el usuario). La
    // intención ya elegida persiste y es barata; solo se ESCALONA la búsqueda
    // de carrera NUEVA: cada jugador busca en ciclos alternos (retraso máximo
    // de 1 ciclo para arrancar, carga simultánea a la mitad).
    if ( ( wm.time().cycle() + wm.self().unum() ) % 2 != 0 )
        return false;
    ...
    // PERF RoboTech: capar candidatos a los 12 más cercanos al arco rival —
    // el costo es lineal en candidatos y los huecos lejanos nunca ganan.
    if ( voronoiPoints.size() > 12 ) {
        std::sort( voronoiPoints.begin(), voronoiPoints.end(),
                   []( const Vector2D & a, const Vector2D & b ) {
                       return a.dist2( Vector2D( 52.5, 0.0 ) )
                            < b.dist2( Vector2D( 52.5, 0.0 ) );
                   } );
        voronoiPoints.resize( 12 );
    }
...
double bhv_scape_voronoi::evaluate_point(rcsc::PlayerAgent *agent, const Vector2D & point,const int & num) {
    ...
    double eval_ball_pass = 0.0; // (port) Setting default: mUseThPassSimInVoroScape=false
